# -*- coding: utf-8 -*-
"""
Created on Sun Jun 20 16:24:47 2021

@author: user
"""

# 底床高程資料(bed elevation)
rtfile = open(r"D:\BF-SPTM\Data\TS_Bottom Elevation.dat",'r') # 輸入檔案(input data) 
...
# 水位高程(water elevation)
rtfile = open(r"D:\BF-SPTM\Data\TS_Water Elevation.dat",'r') # 輸入檔案(input data)
rtlines = rtfile.readlines() #找出對應行數(read data)
rtfile.close()
...
# X方向速度(layer1) velocity in x-direction(layer1)
rtfile = open(r"D:\BF-SPTM\Data\TS_X-Velocity (Layer 1).dat",'r') # 輸入檔案(input data)
rtlines = rtfile.readlines() #找出對應行數(read data)
rtfile.close()
...
# X方向速度(layer2) velocity in x-direction(layer2)
rtfile = open(r"D:\BF-SPTM\Data\TS_X-Velocity (Layer 2).dat",'r') # 輸入檔案(input data)
rtlines = rtfile.readlines() #找出對應行數(read data)
rtfile.close()
...
# X方向速度(layer3) velocity in x-direction(layer3)
rtfile = open(r"D:\BF-SPTM\Data\TS_X-Velocity (Layer 3).dat",'r') # 輸入檔案(input data)
rtlines = rtfile.readlines() #找出對應行數(read data)
rtfile.close()
...
# X方向速度(layer4) velocity in x-direction(layer4)
rtfile = open(r"D:\BF-SPTM\Data\TS_X-Velocity (Layer 4).dat",'r') # 輸入檔案(input data)
rtlines = rtfile.readlines() #找出對應行數(read data)
rtfile.close()
...
# y方向速度(layer1) velocity in y-direction(layer1)
rtfile = open(r"D:\BF-SPTM\Data\TS_Y-Velocity (Layer 1).dat",'r') # 輸入檔案(input data)
rtlines = rtfile.readlines() #找出對應行數(read data)
rtfile.close()
...
# y方向速度(layer2) velocity in y-direction(layer2)
rtfile = open(r"D:\BF-SPTM\Data\TS_Y-Velocity (Layer 2).dat",'r') # 輸入檔案(input data)
rtlines = rtfile.readlines() #找出對應行數(read data)
rtfile.close()
...
# y方向速度(layer3) velocity in y-direction(layer3)
rtfile = open(r"D:\BF-SPTM\Data\TS_Y-Velocity (Layer 3).dat",'r') # 輸入檔案(input data)
rtlines = rtfile.readlines() #找出對應行數(read data)
rtfile.close()
...
# y方向速度(layer4) velocity in y-direction(layer4)
rtfile = open(r"D:\BF-SPTM\Data\TS_Y-Velocity (Layer 4).dat",'r') # 輸入檔案(input data)
rtlines = rtfile.readlines() #找出對應行數(read data)
rtfile.close()
...
# Z方向速度(layer1) velocity in z-direction(layer1)
rtfile = open(r"D:\BF-SPTM\Data\TS_Z-Velocity (Layer 1).dat",'r') # 輸入檔案(input data)
rtlines = rtfile.readlines() #找出對應行數(read data)
rtfile.close()
...
# Z方向速度(layer2) velocity in z-direction(layer2)
rtfile = open(r"D:\BF-SPTM\Data\TS_Z-Velocity (Layer 2).dat",'r') # 輸入檔案(input data)
rtlines = rtfile.readlines() #找出對應行數(read data)
rtfile.close()
...
# Z方向速度(layer3) velocity in z-direction(layer3)
rtfile = open(r"D:\BF-SPTM\Data\TS_Z-Velocity (Layer 3).dat",'r') # 輸入檔案(input data)
rtlines = rtfile.readlines() #找出對應行數(read data)
rtfile.close()
...
# Z方向速度(layer4) velocity in z-direction(layer3)
rtfile = open(r"D:\BF-SPTM\Data\TS_Z-Velocity (Layer 4).dat",'r') # 輸入檔案(input data)
rtlines = rtfile.readlines() #找出對應行數(read data)
rtfile.close()
...
#底床剪應力 (bed shear stress)
rtfile = open(r"D:\BF-SPTM\Data\TS_Total Bed Shear.dat",'r') # 輸入檔案(input data)
rtlines = rtfile.readlines() #找出對應行數(read data)
rtfile.close()
...
#網格距離計算(公尺) (calculate the grid size (unit is meter))
import numpy as np
rtfile = open(r"D:\BF-SPTM\Data\study Grid.xyz",'r') # 輸入檔案(input data)
rtlines = rtfile.readlines() #找出對應行數(read data)
rtfile.close()
...
#輸入資料進入網格(Z軸) (input data into grid(z-axis))
#(底床隨時間改變) (bed will change with time)
start_final = 8
    ...
    for i in range(0,5):
        for n in range(0,3):            
            if x[i] == 0:
                ...
                m =2*i+n
            Z[n,i]=X[n,i]*0+Y[n,i]*0+float(botel[start_final+96*(m)]) #底床(bed)
            Wl[n,i]=X[n,i]*0+Y[n,i]*0+float(wl[start_final+96*(m)])   #水位(water elevation)
...
#建立流速場(x,y,z以及layer1~4) (establish the velocity field, x,y,z with layer1~4)
...
for iteration in range(0,particles):
    ...
    for time in range(0,int(number)):
        ...
        if  time == 0:
                ...
                #限制邊界條件(restrict the boundary condition)                      
                if x[l] == 0:
                    m = k
                else:
                    m =3*l+k                        
...
#計算mean & variance (calculate the mean & variance)
...
# x,z 點分布圖(scatterplot of x,y)
xpoint,ypoint,zpoint =np.ones([particles,1]),np.ones([particles,1]),np.ones([particles,1])
for iteration in range(0,particles):
    xpoint[iteration,0] = locals()['x_backnew'+str(iteration)][-1]
    ypoint[iteration,0] = locals()['y_backnew'+str(iteration)][-1]
    zpoint[iteration,0] = locals()['z_backnew'+str(iteration)][-1]
import numpy as np
fig = plt.figure(figsize=(15,10),dpi=300)
plt.grid(True)
plt.scatter(xpoint,zpoint)
plt.xlabel('X')
plt.ylabel('Z')
plt.title('X,Z scatter')
plt.show()
...
fig = plt.figure(figsize=(15,10),dpi=300)
plt.grid(True)
plt.scatter(ypoint,zpoint)
plt.xlabel('Y')
plt.ylabel('Z')
plt.title('Y,Z scatter')
plt.show()
